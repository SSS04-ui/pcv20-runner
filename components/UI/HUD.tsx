      ...
      <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden border-b-8 border-cyan-500">
        ...
        <div className="space-y-6">
          <div className="flex items-start space-x-4 group">
            <div className="w-12 h-12 rounded-xl bg-cyan-50 flex items-center justify-center text-cyan-600 font-bold border border-cyan-100 group-hover:bg-cyan-100 transition-colors">W</div>
            <div>
              <p className="font-bold text-slate-800">跳躍</p>
              <p className="text-xs text-slate-500">避開地面障礙物 (綠色預警)。空中再次按下可進行二段跳。</p>
            </div>
          </div>
          <div className="flex items-start space-x-4 group">
            <div className="w-12 h-12 rounded-xl bg-rose-50 flex items-center justify-center text-rose-600 font-bold border border-rose-100 group-hover:bg-rose-100 transition-colors">S</div>
            <div>
              <p className="font-bold text-slate-800">滑行</p>
              <p className="text-xs text-slate-500">通過高架雷射門 (紅色預警)。在空中按下可快速降落。</p>
            </div>
          </div>
          <div className="flex items-start space-x-4 group">
            <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-600 font-bold border border-slate-100 group-hover:bg-slate-100 transition-colors">AD</div>
            <div>
              <p className="font-bold text-slate-800">左右移動</p>
              <p className="text-xs text-slate-500">切換賽道以收集疫苗或避開危險。</p>
            </div>
          </div>
        </div>
                 ...
                 <div className="flex flex-col items-center">
                    ...
                    <div className="w-full bg-gradient-to-br from-cyan-50 to-blue-50 rounded-3xl p-8 border border-cyan-100 mb-8 text-center shadow-sm">
                        ...
                        <p className="text-cyan-600 text-[11px] font-bold tracking-widest uppercase bg-white/50 py-3 rounded-2xl border border-cyan-200/50">
                          在游戲中收集 {MAX_VACCINES} 支疫苗，解鎖極限保護吧！
                        </p>
                    </div>
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 md:p-12 z-50">
        ...
        {showLevelUpPopup && (
          <div className="absolute inset-0 flex items-center justify-center z-[600] pointer-events-auto bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
             <div className="w-full max-sm:w-full max-w-sm bg-white p-8 md:p-10 rounded-[2.5rem] border-4 border-orange-500 shadow-2xl text-center transform animate-in zoom-in slide-in-from-bottom-8 duration-500">
                ...
                <h3 className="text-2xl font-black text-slate-900 mb-4 uppercase tracking-tighter">強化目標達成</h3>
        ...
        <div className="flex justify-between items-start w-full">
            <div className="flex flex-col animate-in slide-in-from-left-4 duration-500">
                <div className={`text-4xl md:text-7xl font-black tracking-tighter transition-colors ${isUltimateStage ? 'text-red-500' : 'text-slate-900'}`}>
                    {score.toLocaleString()}
                </div>
                <div className={`text-[10px] font-black tracking-[0.4em] uppercase mt-2 px-1 border-l-2 ml-1 transition-colors ${isUltimateStage ? 'text-red-600 border-red-600' : 'text-cyan-600 border-cyan-500'}`}>已獲得積分</div>
            </div>
            
            <div className="flex items-center space-x-5 pointer-events-auto animate-in slide-in-from-right-4 duration-500">
                <div className={`text-4xl md:text-6xl font-black transition-colors ${timeLeft < 10 ? 'text-red-500 animate-pulse' : (isUltimateStage ? 'text-orange-500' : 'text-slate-900')}`}>
                    {timeLeft.toFixed(1)}<span className="text-lg md:text-2xl ml-1 text-slate-400">秒</span>
                </div>
                
                <button 
                  onClick={() => { audio.init(); togglePause(); }} 
                  className={`p-4 md:p-5 bg-white/80 backdrop-blur-md border rounded-[1.5rem] transition-all active:scale-90 shadow-xl group ${status === GameStatus.PAUSED ? 'bg-cyan-500 border-cyan-500 text-white' : (isUltimateStage ? 'text-red-600 border-red-100' : 'text-slate-800 hover:text-cyan-600 border-slate-200')}`}
                >
                    {status === GameStatus.PAUSED ? <Play className="w-6 h-6 fill-current group-hover:scale-110 transition-transform" /> : <Pause className="w-6 h-6 fill-current group-hover:scale-110 transition-transform" />}
                </button>
            </div>
        </div>
        
        <div className="absolute top-48 md:top-64 left-1/2 transform -translate-x-1/2 flex flex-col items-center w-full max-w-sm px-8 animate-in fade-in duration-700">
             <div className="w-full h-3 bg-slate-200/50 rounded-full overflow-hidden shadow-inner relative border border-white">
             ...
             <div className="flex justify-between w-full mt-3 px-1">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">疫苗進度</span>
                <span className={`text-[11px] font-black uppercase tracking-[0.2em] transition-colors ${isUltimateStage ? 'text-red-600' : 'text-cyan-600'}`}>{vaccineCount} / {MAX_VACCINES}</span>
             </div>
        </div>

        <div className="w-full flex justify-between items-end animate-in slide-in-from-bottom-4 duration-500">
             <div className="bg-white/90 backdrop-blur-sm border border-slate-100 px-6 py-3 rounded-2xl shadow-xl flex flex-col">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.4em] mb-1">權限等級</span>
                <span className={`text-base font-black transition-colors ${isUltimateStage ? 'text-red-600' : 'text-slate-900'}`}>LEVEL {level}</span>
             </div>
             
             <div className="bg-white/90 backdrop-blur-sm border border-slate-100 px-6 py-3 rounded-2xl shadow-xl flex items-center space-x-4">
                 <div className="flex flex-col items-end">
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.4em] mb-1">同步率</span>
                    <span className={`text-base font-black font-mono transition-colors ${isUltimateStage ? 'text-orange-600' : 'text-slate-900'}`}>{(speed / 21.0 * 100).toFixed(0)}%</span>
                 </div>
                 <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isUltimateStage ? 'bg-red-50' : 'bg-cyan-50'}`}>
                    <Zap className={`w-5 h-5 ${isUltimateStage ? 'text-red-500' : 'text-cyan-500'} ${speed > 35 ? 'animate-bounce' : 'animate-pulse'}`} />
                 </div>
             </div>
        </div>
    </div>
