...
export const HUD: React.FC = () => {
  ...
  const TutorialModal = () => (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300 pointer-events-auto">
      <div className="relative w-full max-sm:max-w-[90%] max-w-sm md:max-w-md bg-white rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-12 shadow-2xl border-b-8 border-cyan-500 overflow-hidden">
      ...
      return (
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-slate-50 p-4 pointer-events-auto">
              ...
              <div className="relative w-full max-w-md bg-white border border-slate-100 rounded-[3.5rem] p-8 md:p-14 shadow-2xl animate-in zoom-in-95 duration-500 overflow-y-auto max-h-[95vh] text-center flex flex-col items-center">
                 <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-slate-900 leading-none tracking-tighter mb-10 md:mb-12 whitespace-nowrap">20價大冒險</h1>
                 <div className="w-full bg-gradient-to-br from-cyan-50 to-blue-50 rounded-[2.5rem] p-6 md:p-10 border border-cyan-100 mb-10 shadow-sm flex flex-col items-center space-y-1 md:space-y-2">
                    <p className="text-cyan-900 text-[8.5px] sm:text-[10px] md:text-xs lg:text-sm font-black leading-none whitespace-nowrap overflow-hidden text-center">20價肺炎球菌疫苗是現時全港覆蓋最廣泛的結合疫苗</p>
                    <p className="text-cyan-900 text-[8.5px] sm:text-[10px] md:text-xs lg:text-sm font-black leading-none whitespace-nowrap overflow-hidden text-center">1針長效保護</p>
                    <div className="h-3 md:h-4" />
                    <p className="text-cyan-600 text-[7.5px] sm:text-[9px] md:text-[10px] lg:text-[11px] font-bold tracking-[0.05em] uppercase bg-white/60 py-3 rounded-2xl border border-cyan-200/50 px-4 shadow-sm whitespace-nowrap w-full text-center">
                 ...
                 <div className="w-full space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <button onClick={() => setShowDisclaimer(true)} className="py-5 bg-white border border-slate-100 text-slate-500 font-black rounded-3xl uppercase text-[11px] tracking-widest flex items-center justify-center group hover:bg-slate-50 active:scale-95 transition-all shadow-sm"><Info className="w-5 h-5 mr-3 text-slate-400 group-hover:scale-110 transition-transform" /> 免責聲明</button>
                    </div>
                 </div>
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 md:p-12 z-50">
        ...
        {showLevelUpPopup && (
          <div className="absolute inset-0 flex items-center justify-center z-[1000] pointer-events-auto bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-300">
             <div className="w-full max-sm:max-w-[95%] max-w-sm md:max-w-md bg-white p-8 md:p-14 rounded-[3rem] border-t-[12px] border-orange-500 shadow-2xl text-center transform animate-in zoom-in-95 duration-500">
                <div className="w-16 h-16 md:w-24 md:h-24 bg-orange-50 rounded-[2rem] flex items-center justify-center text-orange-600 mx-auto mb-6 shadow-inner"><ShieldCheck className="w-10 h-10 md:w-12 md:h-12" /></div>
                <h3 className="text-xl md:text-3xl font-black text-slate-900 mb-6 uppercase tracking-tighter whitespace-nowrap">初步目標達成</h3>
                <div className="flex flex-col items-center space-y-1 mb-10 overflow-hidden">
                    <p className="text-[9.5px] sm:text-xs md:text-sm lg:text-lg text-slate-700 font-bold leading-none whitespace-nowrap">20價比15價多30%血清型覆蓋</p>
                    <p className="text-[9.5px] sm:text-xs md:text-sm lg:text-lg text-slate-700 font-bold leading-none whitespace-nowrap">保護更全面</p>
                    <div className="h-2" />
                    <p className="text-[9.5px] sm:text-xs md:text-sm lg:text-lg text-slate-700 font-bold leading-none whitespace-nowrap pt-2">努力取得額外5價保護啦！</p>
                </div>
                <button onClick={() => { audio.init(); dismissMilestone(); }} className="w-full py-5 md:py-6 bg-orange-500 text-white font-black rounded-3xl flex items-center justify-center space-x-4 active:scale-95 transition-all shadow-xl hover:bg-orange-600 text-lg md:text-xl">
        ...
        <div className="flex flex-col w-full space-y-6 md:space-y-10">
            <div className="flex justify-between items-start w-full">
                {/* Score Left */}
                <div className={`text-4xl md:text-7xl font-black tracking-tighter transition-colors drop-shadow-md ${isUltimateStage ? 'text-red-500' : 'text-sky-400'}`}>
                    {score.toLocaleString()}
                </div>
                
                {/* Timer and Pause Right */}
                <div className="flex items-center space-x-4 md:space-x-6 pointer-events-auto">
                    <div className={`text-3xl md:text-6xl font-black transition-colors drop-shadow-md text-white ${timeLeft < 10 ? 'animate-pulse' : ''}`}>
                        {timeLeft.toFixed(1)}<span className={`text-sm md:text-2xl ml-2 font-bold uppercase text-white/70`}>Sec</span>
                    </div>
                    <button 
            ...
            <div className="w-full flex flex-col items-center">
                ...
                <div className="flex justify-between w-full max-w-xl mt-3 px-2">
                    <span className="text-[9px] md:text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] drop-shadow-sm">疫苗進度</span>
