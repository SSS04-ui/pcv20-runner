  ...
  if (status === GameStatus.GAME_OVER || status === GameStatus.VICTORY) {
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-[500] bg-slate-950/80 backdrop-blur-xl p-4 pointer-events-auto animate-in fade-in duration-500">
        <div className={`relative w-full max-sm:max-w-[90%] max-w-sm bg-white border-t-8 ${isVictory ? 'border-emerald-500' : 'border-rose-500'} rounded-[3rem] p-8 md:p-10 flex flex-col items-center shadow-2xl animate-in slide-in-from-bottom-12 duration-700`}>
          <div className={`w-20 h-20 md:w-24 md:h-24 rounded-3xl ${isVictory ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'} flex items-center justify-center mb-6 shadow-sm rotate-3 animate-bounce`}>
            {isVictory ? <CheckCircle2 className="w-12 h-12 md:w-14 md:h-14" /> : <AlertTriangle className="w-12 h-12 md:w-14 md:h-14" />}
          </div>
          <div className="text-center mb-10">
            <h2 className={`text-3xl md:text-4xl font-black ${isVictory ? 'text-emerald-600' : 'text-rose-600'} tracking-tighter uppercase leading-none`}>
              {isVictory ? '任務成功' : '任務失敗'}
            </h2>
            <div className="text-xs md:text-sm font-bold text-slate-500 mt-4 leading-relaxed bg-slate-50 px-4 md:px-6 py-4 rounded-2xl border border-slate-100 shadow-inner">
          ...
          <div className="w-full space-y-4">
            <button onClick={() => { audio.init(); restartGame(); }} className={`w-full group py-6 ${isVictory ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-slate-900 hover:bg-black'} text-white font-black rounded-3xl transition-all shadow-xl active:scale-95 flex items-center justify-center space-x-3`}>
              <RefreshCcw className="w-5 h-5 md:w-6 md:h-6 group-hover:rotate-180 transition-transform duration-500" />
              <span className="text-lg md:text-xl tracking-widest uppercase">{isVictory ? '重新開始' : '重新部署'}</span>
            </button>
            <button onClick={() => { audio.init(); setStatus(GameStatus.MENU); }} className="w-full py-4 bg-white border-2 border-slate-100 text-slate-400 font-bold rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-center space-x-3 active:scale-95">
              ...
              <span className="tracking-widest uppercase text-[10px] md:text-xs font-black">返回主頁</span>
            </button>
          </div>
        </div>
      </div>
  ...
  if (status === GameStatus.MENU) {
      return (
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-slate-50 p-4 pointer-events-auto">
              <div className="relative w-full max-w-md bg-white border border-slate-100 rounded-[3.5rem] p-8 md:p-14 shadow-2xl animate-in zoom-in-95 duration-500 overflow-y-auto max-h-[95vh]">
                 <div className="flex flex-col items-center">
                    ...
                    <div className="w-full bg-gradient-to-br from-cyan-50 to-blue-50 rounded-3xl p-6 md:p-8 border border-cyan-100 mb-6 md:mb-8 text-center shadow-sm">
                    ...
                    <div className="w-full space-y-3">
                        ...
                        <button onClick={() => { audio.init(); setStatus(GameStatus.STATS); }} className="w-full py-4 bg-white border border-slate-100 text-slate-500 font-black rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-center tracking-widest uppercase text-xs">
                           任務日誌
                        </button>
                    </div>
                 </div>
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 md:p-12 z-50">
        ...
        {showLevelUpPopup && (
          <div className="absolute inset-0 flex items-center justify-center z-[600] pointer-events-auto bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
             <div className="w-full max-sm:max-w-[90%] max-w-sm bg-white p-8 md:p-10 rounded-[2.5rem] border-4 border-orange-500 shadow-2xl text-center transform animate-in zoom-in duration-500">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-orange-100 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-inner">
                   <ShieldCheck className="w-10 h-10 md:w-12 md:h-12 text-orange-600" />
                </div>
                <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-4 uppercase tracking-tighter">初步目標達成</h3>
                <p className="text-sm md:text-lg text-slate-700 font-bold leading-relaxed mb-8">
                  20價比15價多30%血清型覆蓋，保護更全面，努力取得最後 {MAX_VACCINES - MILESTONE_VACCINE_COUNT} 支達成升級防禦
                </p>
                <button onClick={() => { audio.init(); dismissMilestone(); }} className="w-full py-4 md:py-5 bg-orange-500 hover:bg-orange-600 text-white font-black text-lg md:text-xl rounded-2xl transition-all shadow-xl active:scale-95 flex items-center justify-center space-x-3 group">
                  <span>繼續前進</span>
                  <FastForward className="w-5 h-5 md:w-6 md:h-6 group-hover:translate-x-1 transition-transform" />
                </button>
             </div>
          </div>
        )}
        ...
        <div className="w-full flex justify-between items-end pb-safe">
             <div className="bg-white/90 backdrop-blur-sm border border-slate-100 px-4 py-2 rounded-xl shadow-xl flex flex-col">
             ...
             <div className="bg-white/90 backdrop-blur-sm border border-slate-100 px-4 py-2 rounded-xl shadow-xl flex items-center">
