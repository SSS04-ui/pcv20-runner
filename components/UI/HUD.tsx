  ...
  const VersionFooter = () => (
    <div className="absolute bottom-4 left-0 right-0 flex justify-center opacity-30 pointer-events-none text-center px-4">
        <div className="flex items-center space-x-2 font-mono text-[8px] md:text-[9px] tracking-[0.2em] text-cyan-800 uppercase">
            ...
            <span>20價大冒險 v5.5.0 | 任務控制</span>
        </div>
    </div>
  );

  const ModalContainer = ({ children, onClose, borderClass = "border-cyan-500" }: { children: React.ReactNode, onClose: () => void, borderClass?: string }) => (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300 pointer-events-auto">
      <div className={`relative w-full max-w-lg bg-white rounded-[2.5rem] p-8 md:p-14 shadow-2xl border-t-8 ${borderClass} overflow-hidden flex flex-col max-h-[92vh]`}>
          ...
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-slate-900/10 p-4 pointer-events-auto backdrop-blur-[2px]">
              ...
              <div className="relative w-full max-w-lg md:max-w-4xl bg-white/95 border border-white/50 rounded-[3.5rem] p-8 md:p-16 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.2)] animate-in zoom-in-95 duration-700 overflow-y-auto max-h-[96vh] text-center flex flex-col items-center scrollbar-hide">
                 ...
                 <div className="w-full bg-gradient-to-br from-cyan-50 to-indigo-50/50 rounded-[2.5rem] p-8 md:p-12 border border-blue-100 mb-8 md:mb-12 flex flex-col items-center space-y-6 md:space-y-10">
                    <p className="text-slate-900 text-xl sm:text-3xl md:text-5xl font-black leading-tight text-center">
                    ...
                    <div className="flex items-center space-x-6 bg-slate-900 text-white px-8 py-4 md:px-14 md:py-8 rounded-3xl shadow-xl transition-all hover:scale-105 active:scale-95">
                        ...
                        <p className="text-xl sm:text-4xl md:text-6xl font-black font-cyber tracking-wide whitespace-nowrap">1針長效保護</p>
                    </div>
                 ...
                 <div className="w-full space-y-4 md:space-y-6 max-w-xl">
                    <button onClick={() => { audio.init(); startGame(); }} className="group w-full py-6 md:py-9 bg-slate-900 text-white font-black text-2xl md:text-4xl rounded-3xl hover:bg-black transition-all shadow-2xl flex items-center justify-center uppercase tracking-widest active:scale-95 font-cyber">
                        啟動任務 <Play className="ml-5 w-6 h-6 md:w-10 md:h-10 fill-white" />
                    </button>
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 md:p-12 z-50">
        ...
        {isMilestonePaused && countdownValue > 0 && (
            <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-slate-900/10 backdrop-blur-[1px]">
                <div className="text-[10rem] md:text-[25rem] font-black text-white font-cyber animate-ping drop-shadow-[0_0_50px_rgba(14,165,233,0.8)]">
                    {countdownValue}
                </div>
            </div>
        )}
        ...
        {showLevelUpPopup && (
          <div className="absolute inset-0 flex items-center justify-center z-[1000] pointer-events-auto bg-slate-950/70 backdrop-blur-xl animate-in fade-in duration-300">
             <div className="w-full max-sm:max-w-[95%] max-w-sm md:max-w-md bg-white p-10 md:p-16 rounded-[4rem] border-t-[16px] border-orange-500 shadow-2xl text-center transform animate-in zoom-in-95 duration-500">
                ...
                <h3 className="text-3xl md:text-5xl font-black text-slate-900 mb-8 uppercase tracking-tighter">繼續努力</h3>
                <div className="space-y-4 mb-12">
                    <p className="text-xl md:text-3xl text-slate-700 font-black leading-none">20價比15價多<span className="text-orange-600">30%+</span>覆蓋</p>
                    <p className="text-lg md:text-2xl text-slate-600 font-bold">全面守護，守護更多！</p>
                </div>
                <button onClick={() => { audio.init(); dismissMilestone(); }} className="w-full py-6 bg-orange-500 text-white font-black rounded-3xl flex items-center justify-center space-x-6 active:scale-95 transition-all text-xl md:text-2xl uppercase tracking-widest shadow-xl">
                  <span>獲取額外5價保護</span>
        ...
        {status === GameStatus.GAME_OVER || status === GameStatus.VICTORY ? (
          <div className="absolute inset-0 flex items-center justify-center z-[500] bg-slate-950/80 backdrop-blur-2xl p-6 pointer-events-auto animate-in fade-in duration-500">
            <div className={`relative w-full max-sm:max-w-[95%] max-w-md bg-white border-t-[12px] ${status === GameStatus.VICTORY ? 'border-emerald-500' : 'border-rose-500'} rounded-[4rem] p-12 flex flex-col items-center shadow-2xl text-center`}>
              ...
              <h2 className={`text-4xl md:text-5xl font-black ${status === GameStatus.VICTORY ? 'text-emerald-600' : 'text-rose-600'} uppercase mb-8 tracking-tighter`}>
                {status === GameStatus.VICTORY ? '20價疫苗get' : '任務失敗'}
              </h2>
              <div className="text-lg md:text-xl font-bold text-slate-500 mb-12 bg-slate-50 px-8 py-6 rounded-[2rem] border border-slate-100 shadow-inner leading-snug">
                {status === GameStatus.VICTORY ? `成功獲得最廣泛保護` : `未能收集完整 20 價保護，重新嘗試！`}
              </div>
              <div className="w-full space-y-4">
                <button onClick={() => { audio.init(); restartGame(); }} className={`w-full py-6 md:py-8 ${status === GameStatus.VICTORY ? 'bg-emerald-600' : 'bg-slate-900'} text-white font-black rounded-3xl shadow-xl flex items-center justify-center space-x-4 active:scale-95 text-xl md:text-2xl uppercase`}>
        ...
        {/* HUD In-Game Elements - Reduced Mobile Font Sizes (~30% reduction) */}
        <div className="flex flex-col w-full space-y-8 md:space-y-12">
            <div className="flex justify-between items-start w-full">
                <div className={`text-3xl md:text-[12rem] font-black tracking-tighter transition-colors drop-shadow-lg font-cyber whitespace-nowrap ${isUltimateStage ? 'text-red-500 glitch-text' : 'text-sky-400'}`}>
                    {score.toLocaleString()}
                </div>
                
                <div className="flex items-center space-x-4 md:space-x-6 pointer-events-auto">
                    <div className={`text-2xl md:text-[9rem] font-black transition-colors drop-shadow-md text-white font-cyber whitespace-nowrap ${timeLeft < 10 ? 'animate-pulse text-red-500' : ''}`}>
                        {timeLeft.toFixed(1)}<span className="text-xs md:text-5xl ml-1 md:ml-2 font-bold opacity-60">S</span>
                    </div>
                    <button onClick={() => { audio.init(); togglePause(); }} className="p-3 md:p-10 bg-white/95 rounded-2xl md:rounded-[4rem] shadow-2xl active:scale-90 transition-transform">
            ...
            <div className="w-full flex flex-col items-center">
                <div className="w-full max-w-5xl h-4 md:h-10 bg-slate-200/30 rounded-full overflow-hidden shadow-inner border-[2px] md:border-[4px] border-white/20 relative backdrop-blur-md">
                ...
                <div className="flex justify-between items-center w-full max-w-5xl mt-2 md:mt-4 px-2 md:px-4">
                    <div className="flex items-center space-x-2 md:space-x-4">
                      <span className="text-[10px] md:text-3xl font-black text-white uppercase tracking-widest font-cyber whitespace-nowrap">疫苗進度</span>
                      {isUltimateStage && (
                        <div className="px-2 py-0.5 bg-red-600 text-white text-[8px] md:text-xl font-black rounded md:rounded-lg animate-pulse uppercase tracking-tighter shadow-[0_0_20px_rgba(220,38,38,0.8)]">
                           Ultimate Stage
                        </div>
                      )}
                    </div>
                    <span className={`text-[10px] md:text-4xl font-black tracking-widest font-cyber whitespace-nowrap ${isUltimateStage ? 'text-red-400' : 'text-cyan-300'}`}>{vaccineCount} / 20 價</span>
                </div>
            </div>
        </div>
        ...
        <div className="w-full flex justify-between items-end pb-safe">
             <div className={`bg-white/95 px-6 py-3 md:px-16 md:py-10 rounded-2xl md:rounded-[2.5rem] text-sm md:text-6xl font-black font-cyber ${isUltimateStage ? 'text-red-600' : 'text-slate-900'} shadow-2xl tracking-widest uppercase whitespace-nowrap`}>
                LVL {level}
             </div>
             <div className="flex space-x-3 md:space-x-10">
                 <div className="bg-white/95 p-4 md:p-12 rounded-2xl md:rounded-[4rem] shadow-2xl border border-white/50">
                    <Zap className={`w-6 h-6 md:w-20 md:h-20 ${isUltimateStage ? 'text-red-500' : 'text-cyan-500'}`} />
                 </div>
                 <div className="bg-white/95 p-4 md:p-12 rounded-2xl md:rounded-[4rem] shadow-2xl border border-white/50">
                    <ShieldCheck className={`w-6 h-6 md:w-20 md:h-20 ${isUltimateStage ? 'text-orange-500' : 'text-slate-400'}`} />
                 </div>
             </div>
        </div>
    </div>
