...
export const HUD: React.FC = () => {
  ...
  const PauseOverlay = () => (
    <div className="absolute inset-0 z-[800] bg-slate-950/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300 pointer-events-auto">
      <div className="w-full max-sm:w-full max-w-sm bg-white rounded-[3rem] p-10 shadow-2xl border-b-8 border-cyan-500 relative overflow-hidden flex flex-col items-center text-center">
        <div className="w-16 h-16 bg-cyan-50 rounded-2xl flex items-center justify-center text-cyan-600 mb-6 shadow-inner">
          <Pause className="w-8 h-8 fill-current" />
        </div>
        <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-6">任務暫停</h2>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-[120] bg-slate-950/60 backdrop-blur-xl p-4 pointer-events-auto">
        <div className="w-full max-w-xl bg-white border-2 border-cyan-100 rounded-[2.5rem] p-8 md:p-12 shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between mb-8">
            ...
            <h2 className="text-3xl font-black text-slate-800 tracking-tighter uppercase">任務日誌</h2>
          ...
          <button onClick={() => setStatus(GameStatus.MENU)} className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl hover:bg-black transition-all">返回基地</button>
        </div>
      </div>
    );
  }
  ...
  if (status === GameStatus.GAME_OVER || status === GameStatus.VICTORY) {
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-[500] bg-slate-950/80 backdrop-blur-xl p-4 pointer-events-auto animate-in fade-in duration-500">
        <div className={`relative w-full max-w-sm bg-white border-t-8 ${isVictory ? 'border-emerald-500' : 'border-rose-500'} rounded-[3rem] p-10 flex flex-col items-center shadow-2xl text-center`}>
          <div className={`w-20 h-20 md:w-24 md:h-24 rounded-3xl ${isVictory ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'} flex items-center justify-center mb-6`}>
            {isVictory ? <CheckCircle2 className="w-14 h-14" /> : <AlertTriangle className="w-14 h-14" />}
          </div>
          <h2 className={`text-3xl md:text-4xl font-black ${isVictory ? 'text-emerald-600' : 'text-rose-600'} uppercase mb-4`}>
            {isVictory ? '任務成功' : '任務失敗'}
          </h2>
      ...
      return (
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-slate-50 p-4 pointer-events-auto">
              <div className="relative w-full max-w-md bg-white border border-slate-100 rounded-[3.5rem] p-8 md:p-14 shadow-2xl animate-in zoom-in-95 duration-500 overflow-y-auto max-h-[90vh] text-center">
                 ...
                 <div className="w-full bg-gradient-to-br from-cyan-50 to-blue-50 rounded-3xl p-6 md:p-8 border border-cyan-100 mb-8 shadow-sm">
        ...
        {showLevelUpPopup && (
          <div className="absolute inset-0 flex items-center justify-center z-[600] pointer-events-auto bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
             <div className="w-full max-sm:max-w-[90%] max-w-sm bg-white p-8 md:p-10 rounded-[2.5rem] border-4 border-orange-500 shadow-2xl text-center transform animate-in zoom-in duration-500">
                ...
                <p className="text-sm md:text-lg text-slate-700 font-bold mb-8">
                  20價比15價多30%血清型覆蓋，保護更全面，努力取得最後 {MAX_VACCINES - MILESTONE_VACCINE_COUNT} 支達成升級防禦
                </p>
                <button onClick={() => { audio.init(); dismissMilestone(); }} className="w-full py-4 md:py-5 bg-orange-500 text-white font-black rounded-2xl flex items-center justify-center space-x-3">
                  <span>繼續前進</span>
